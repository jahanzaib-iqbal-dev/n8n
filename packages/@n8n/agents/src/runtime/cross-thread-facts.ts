...
const DEFAULT_EMBEDDER = 'openai/text-embedding-3-small';
const RRF_K = 60;
const MS_PER_DAY = 24 * 60 * 60 * 1000;

const DEFAULT_EXTRACTION_PROMPT = `Extract durable, user-specific facts from the transcript.
...
export function withCrossThreadFactDefaults(
	...
	return {
		...
		embedder: config.embedder ?? DEFAULT_EMBEDDER,
...
export async function extractAndStoreCrossThreadFacts(
	...
	try {
		...
		const rows: NewCrossThreadFact[] = facts.map((content, index) => ({
			...
			embeddingModel: normalized.embedder,
		}));
...
async function embedValues(
	...
	const embeddingModel = createEmbeddingModel(config.embedder, config.apiKey);
	const result = await embedMany({ model: embeddingModel, values });
