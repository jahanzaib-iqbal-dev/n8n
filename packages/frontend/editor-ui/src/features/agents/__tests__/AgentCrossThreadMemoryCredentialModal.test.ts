...
import AgentCrossThreadMemoryCredentialModal from '../components/AgentCrossThreadMemoryCredentialModal.vue';
...
const MODAL_NAME = 'agentCrossThreadMemoryCredentialModal';

function renderModal({ onSelect = vi.fn() }: { onSelect?: (credentialId: string) => void } = {}) {
	const renderComponent = createComponentRenderer(AgentCrossThreadMemoryCredentialModal, {
		global: {
			stubs: {
				Modal: ModalStub,
...
describe('AgentCrossThreadMemoryCredentialModal', () => {
	...
	it('keeps confirm disabled until an OpenAI credential is selected', async () => {
		...
		expect(getByTestId('agent-cross-thread-memory-credential-picker')).toHaveAttribute(
			'data-credential-type',
			'openAiApi',
		);
		expect(getByTestId('agent-cross-thread-memory-credential-confirm')).toBeDisabled();

		await fireEvent.click(getByTestId('select-credential'));
		await fireEvent.click(getByTestId('agent-cross-thread-memory-credential-confirm'));
	...
	it('closes without selecting a credential when canceled', async () => {
		...
		await fireEvent.click(getByTestId('agent-cross-thread-memory-credential-cancel'));
